...
class Model {
    ...
    func render(renderEncoder: MTLRenderCommandEncoder) {
        ...
        renderEncoder.setVertexBytes(&model, length: MemoryLayout.stride(ofValue: model), index: 2)
        ...
        for mesh in self.meshes {
            ...
            for (submesh, material) in zip(mesh.mesh.submeshes, mesh.materials) {
                ...
                renderEncoder.setFragmentBytes(&materialInfo, length: MemoryLayout.stride(ofValue: materialInfo), index: 1)
